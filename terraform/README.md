...
- `.github/workflows/build-orm-stack-zip.yml` -> builds `oci-deploy-selfhosted-mcp-oke-latest.zip`
...
The workload identity policy is always created automatically for the generated service accounts.
The stack creates OCI DevOps resources for both the MCP server and MCP client. The initial OCI DevOps build run is opt-in and defaults to `false` so Resource Manager apply is not blocked by post-provision build readiness.
...
This repo is now Resource Manager-ready directly from source control:

- working directory: `terraform`
- schema file: `terraform/schema.yaml`
- current OCI DevOps branch for this repo: `master`
...
4. Choose **DevOps**
5. Select the `hosted-mcp-oke` project and the `selfhosted-mcp-oke` repository
...
- the stack apply succeeds without forcing the first build run
- if `devops_trigger_initial_build = true`, the first build run also starts from the same apply
...
- branch `master` exists in that repository
- `devops_repository_id` and `devops_repository_url` point to that same repository
- `current_user_ocid` is available to Resource Manager, and `ocir_auth_token` is only needed if the current user already has 2 auth tokens
...
Expected quick-create URL format:

```text
https://cloud.oracle.com/resourcemanager/stacks/create?region=home&zipUrl=https://github.com/<org>/<repo>/releases/latest/download/oci-deploy-selfhosted-mcp-oke-latest.zip
```

The `zipUrl` must be a direct URL to a `.zip` file. A repository folder URL cannot be used as the quick-create target.
